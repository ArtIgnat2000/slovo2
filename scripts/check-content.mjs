 ...
 *   • опасной отмечено ТОЛЬКО то, где написание расходится со звучанием:
 *     все безударные о/е/я/э (они слышатся иначе) — и только они,
 *     безударные а/и/у/ы/ю слышатся как пишутся и помечены быть не должны;
...
/**
 * Безударные гласные, которые слышатся не так, как пишутся (о→а, е/я/э→и):
 * именно их помечаем как опасные — и только их. Остальные безударные гласные
 * слышатся как пишутся, подсвечивать их не нужно (в «ученица» опасна только Е).
 */
const TRICKY = 'оеяэ';
...
for (const row of rows) {
  ...
  // подсвечиваем только то, где написание расходится со звучанием:
  // безударные о/е/я/э обязаны быть помечены, а гласные, которые слышатся
  // как пишутся (а/и/у/ы/ю), помечены быть не должны
  letters.forEach((ch, i) => {
    if (i === stress || !VOWELS.includes(ch)) return;
    const tricky = TRICKY.includes(ch);
    const marked = danger.includes(i);
    if (tricky && !marked)
      fail(text, `безударная «${ch}» (${i}) слышится иначе — её надо отметить опасной`);
    if (!tricky && marked)
      fail(text, `безударная «${ch}» (${i}) слышится так, как пишется — её подсвечивать не нужно`);
  });
